<template>
  <view class="page">
    <view v-if="list.length === 0" class="empty">暂无报名记录</view>
    <view v-for="item in list" :key="item.id" class="card">
      <text class="title">活动ID：{{ item.activityId }}</text>
      <text class="sub">报名时间：{{ item.createTime }}</text>
...
onMounted(async () => {
  ...
  try {
    const res = await get('/api/activity-registration/my/' + u.userId)
    list.value = res.data || []
  } catch (e) {}
})
</script>
...
.sub { font-size: 24rpx; color: var(--text-secondary); display: block; margin-bottom: 8rpx; }
